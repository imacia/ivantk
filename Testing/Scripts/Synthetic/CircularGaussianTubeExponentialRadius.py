...
gaussian_tube_exe_str = exe_path_str + 'TestCircularGaussianTube.exe'
noise_exe_str = exe_path_str + 'UtilityGaussianNoiseAdder.exe'
gaussian_tube_output_str_root = output_path_str + 'TestObjects/Normalized/' + 'CircularGaussianTube_'
...
for i in range( 0, numScales ):
  ...
  gaussian_tube_output_str = gaussian_tube_output_str_root + \
    str( int( tubeScaleModf[1] ) ) + '_' + str( int( tubeScaleModf[0] * 100.0 ) ) + '.mhd'
    
  print gaussian_tube_exe_str, gaussian_tube_output_str, str( currentScale ), tubeHeightStr, \
    str( normalize ), str( maxValue ), str( tubeSpacing ), str( sectionImageSize ), str( writeCenterline )
  
  subprocess.call( [ gaussian_tube_exe_str, gaussian_tube_output_str, str( currentScale ), tubeHeightStr, \
    str( normalize ), str( maxValue ), str( tubeSpacing ), str( sectionImageSize ), str( writeCenterline ) ] )
