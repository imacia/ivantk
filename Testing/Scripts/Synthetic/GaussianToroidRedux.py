...
#toroidScales = [0.75, 1.0, 1.25, 1.5, 2.0, 3.0, 5.0, 7.5, 10.0]
#toroidRadii  = [1.0, 2.5, 5.0, 10.0, 25.0, 50.0, 100.0, 250.0]
toroidScales = [1.0, 5.0, 10.0]
toroidRadii  = [2.5, 10.0, 50.0, 250.0]
...
for i in range( len( toroidScales ) ):
  ...
  for j in range( len( toroidRadii ) ):
    ...
    print gaussian_toroid_exe_str, gaussian_toroid_output_str, str( toroidScales[i] ), str( toroidRadii[j] ), '2', str( toroidSpacing ), str( startAngle ), str( endAngle ), str( maxValue ), fullCircle
    subprocess.call( [ gaussian_toroid_exe_str, gaussian_toroid_output_str, str( toroidScales[i] ), str( toroidRadii[j] ), '2', str( toroidSpacing ), str( startAngle ), str( endAngle ), str( maxValue ), str( fullCircle ) ] )
